...
export default function EditFeeStructurePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        // Only active Fee Types can be selected
        ...
        if (storedStructs) {
            try {
                ...
                if (current) {
                    ...
                    setGradeClass(current.gradeClass);
                    setCurrency(current.currency);
                    setActive(current.active);
                    setSelectedItems(current.items.map(item => ({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const updated = structures.map(item => {
            if (item.id === id) {
                return {
                    ...item,
                    name: name.trim(),
        ...
        <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-300">
            ...
            <div>
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-tighter text-slate-700">Edit Structure</span>
                </div>
                <h2 className="text-3xl font-black tracking-tight text-slate-900 uppercase italic">Edit Fee Structure</h2>
                <p className="text-slate-500 font-medium">Modify existing bundled billing allocations and fee items.</p>
            </div>
